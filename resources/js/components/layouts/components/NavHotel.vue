<template>
    <v-list-item-group v-model="selectedItem" color="accent">
        <v-list-item link to="/dashboard" color="accent">
            <v-list-item-action>
        ...
        <v-list-item link to="/branchHotelSystem" color="accent">
            <v-list-item-action>
            <v-icon>mdi-room-service-outline</v-icon>
            </v-list-item-action>
            <v-list-item-content>
            <v-list-item-title><b>Recepción</b></v-list-item-title>
            </v-list-item-content>
        </v-list-item>
        <v-list-item link to="/bookings" color="accent">
            <v-list-item-action>
            <v-icon>mdi-book-multiple-outline</v-icon>
            </v-list-item-action>
            <v-list-item-content>
            <v-list-item-title><b>Reservas</b></v-list-item-title>
            </v-list-item-content>
        </v-list-item>

        <v-list-group prepend-icon="mdi-cash-register" color="accent" v-if="hasMenuAccess(process_hotel_cash)">
            <template v-slot:activator>
            <v-list-item-title><b>Caja</b></v-list-item-title>
            </template>
            <template v-for="([title, icon, link, gate], i) in process_hotel_cash">
                <v-list-item
                    v-bind:key="i"
                    link
                    :to="'/'+link"
        ...
        <v-divider></v-divider>
        <v-list-group prepend-icon="mdi-account-group-outline" color="accent" v-if="hasMenuAccess(maestros_hotel_admin)">
            <template v-slot:activator>
            <v-list-item-title><b>Administración</b></v-list-item-title>
            </template>
            <template v-for="([title, icon, link, gate],i) in maestros_hotel_admin">
                <v-list-item
                    link
                    v-bind:key="i"
                    :to="'/'+link"
        ...
        <v-list-group prepend-icon="mdi-office-building" color="accent" v-if="hasMenuAccess(maestros_hotel_logistic)">
            <template v-slot:activator>
            <v-list-item-title><b>Logística</b></v-list-item-title>
            </template>
            <template v-for="([title, icon, link, gate],i) in maestros_hotel_logistic">
                <v-list-item
                    link
                    v-bind:key="i"
                    :to="'/'+link"
        ...
        <v-list-group prepend-icon="mdi-cog-outline" color="accent" v-if="hasMenuAccess(maestros_config)">
            <template v-slot:activator>
...
export default {
    data: () => ({
        ...
        process_cash: [
            ['Caja', 'mdi-inbox-arrow-up-outline','cash/hotel','"read_cash"'],
            ['Egresos', 'mdi-inbox-arrow-down-outline','expenses/hotel','read_cash'],
        ],
        maestros_config: [
        ...
        maestros_hotel_logistic: [
            ['Habitaciones', 'mdi-bed-empty', 'rooms','read_rooms'],
            ['Tipo Habitación', 'mdi-clipboard-file', 'type_room','read_type_room'],
            ['Zonas', 'mdi-home-map-marker', 'zones_hotel','read_zone_hotel'],
