<template>
  <div class="flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <h2 style="color: #37474F">{{mesa.zona.nombre}} - {{mesa.nro_mesa}} | Mozo: {{ nombre_mozo }}</h2>
      </div>
      <v-spacer></v-spacer>
      <v-tooltip bottom>
    ...
    <div class="my-2">
        <div>
                            ...
                            <v-tabs-items v-model="tab">
                                ...
                                <v-card flat style="height:100%;margin-top: 10px;">
                                        ...
                                        <template v-slot:default>
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width:20%;" class="text-left">CNT</th>
                                            ...
                                            <tbody>
                                                ...
                                                >
                                                    ...
                                                    <td>
                                                        <div><b>{{ item.producto.nombre_producto }}</b></div>
                                                        <span style="font-size: 0.8rem;"> {{item.detalle_combo_descripcion}}</span>
                                                    </td>
                                                    <td> 
                                                        <v-row no-gutters>
                                                            <v-col
                                    ...
                                    <v-card-actions class="justify-end" >
                                        <v-btn color="error" icon @click="deleteMesaOrden(form.orden.id_orden)">
                                        <v-icon>mdi-delete-outline</v-icon>
                                        </v-btn> 
                                        <v-spacer></v-spacer>
                                        <v-btn color="error"  @click="deleteMesaOrden(form.orden.id_orden)" v-if="form.orden.detalle.length>0">
                                        <v-icon>mdi-close</v-icon> Cancelar
                                        </v-btn>
                                        <v-btn color="info" v-if="form.orden.detalle.length>0" @click="printCuenta(form.orden.id_orden)">
                                        <v-icon>mdi-printer-pos</v-icon> Imprimir Cuenta
                                        </v-btn>
                                        <v-btn color="warning" v-if="form.orden.detalle.length>0"  @click="printCocina()">
                                        <v-icon>mdi-chef-hat</v-icon> Enviar Cocina</v-btn>
                                        <v-btn color="success" v-if="form.orden.detalle.length>0" @click="preLoadData">
            ...
            <v-dialog v-model="comprobanteDialog" max-width="500px" persistent>
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        ></v-select>
                        <v-text-field
                            v-if="generate_sales.header.id_medio_pago==3 || generate_sales.header.id_medio_pago==5"
                            label="Nro. Operación"
                            hide-details="auto"
                    ...
                    <v-card-actions>
                        ...
                        >Cancelar</v-btn
                        >
                        <v-btn color="primary" :disabled="load_serie" :loading="loading" @click="saveComprobante"
                        >Guardar</v-btn
                        >
                        <v-spacer></v-spacer>
                    </v-card-actions>
            ...
            <v-dialog v-model="adicionalDialog" max-width="500px" persistent>
                <v-card>
                    <v-card-title class="subtitle-1">Detalle del Combo</v-card-title>
                    <v-card-text>
                        <v-list
                            subheader
                            >
                            <v-subheader>Productos</v-subheader>
                            <template  v-for="item in comboDetail">
                                <v-list-item
                                ...
                                >
                                    ...
                                    <v-list-item-content>
                                    <v-list-item-title v-text="item.producto.nombre_producto"></v-list-item-title>
                                    </v-list-item-content>
                                </v-list-item>
                            </template>
                            <template  v-for="item in comboDetail">
                                <section :key="item.id_adicional" v-if="!item.id_producto">
                                    <v-divider></v-divider>
                                    <v-subheader>{{item.adicional.nombre}}</v-subheader>
                                    <v-list-item>
                                        <v-radio-group
                                            column
                                            v-model="item.select"
                                            >
                                            <v-radio v-for="(adicional,index) in item.adicional.detalle"
                                                :label="`${Number(adicional.cantidad)} ${adicional.producto.nombre_producto} (+ S/.${adicional.precio}) `"
                                                :value="adicional"
                                                :key="index"
                                            ></v-radio>
                                        </v-radio-group>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="error" dark @click="adicionalDialog=false"
                        >Cancelar</v-btn
                        >
                        <v-btn color="primary" :disabled="adicionalLoading" :loading="adicionalLoading" @click="saveCombo"
                        >Guardar</v-btn
                        >
                        <v-spacer></v-spacer>
                    </v-card-actions>
                ...
                >
                    <v-container>
                        ...
                        <v-col cols="12" sm="6"  md="6" >
                            ...
                            ></v-text-field>
                        <v-text-field v-model="nuevoCliente.email" label="Correo Electrónico" placeholder="Correo Electrónico" :rules="requiredRules"
                            ></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6"  md="6" ><!--v-on:keyup.enter="buscarDoc()"-->
                            ...
                            ></v-text-field>
                            <v-text-field v-model="nuevoCliente.telefono" label="Teléfono" placeholder="Teléfono" :rules="requiredRules"
                            ></v-text-field>
                        </v-col>
                        </v-row>
                    </v-container>
  ...
  methods: {
        async toggleSearchCsutomer(){
            ...
            if(rpta.data.success === true){
                ...
                console.log(this.generate_sales.header.nombre_cliente)
            }else{
                this.$store.commit('LOADER',false);
        ...
        getTiposComprobante(){
            ...
            if(tiposComprobanteLocal == null){
                ...
                //console.log( this.tipo_comprobante[2].id_tipo_comprobante);
                this.generate_sales.header.id_tipo_comprobante = this.tipo_comprobante[2].id_tipo_comprobante;
                //this.$store.commit('LOADER',false);
            }
        ...
        async getOrdenesMesa(){
            //ORDENES
            await axios.get('/api/mesaorden/'+this.id_mesa)
            .then((response) => { 
                this.tabs_form = [];
                ...
                for (let index = 0; index < this.tabs_form.length; index++) {
                    ...
                    //Seleccionamos mozo
                    let id_mozo = this.tabs_form[index].orden.id_usuario;
                    let mozos = this.mozos;
                    let obj = mozos.filter(item => item.id === id_mozo);
                    this.nombre_mozo = obj[0].name;
                    ...
                    for (let j = 0; j < this.tabs_form[index].orden.detalle.length; j++) {
                        ...
                        if(this.tabs_form[index].orden.detalle[j].detalle_combo)
                        {
                            this.tabs_form[index].orden.detalle[j].detalle_combo_descripcion =((JSON.parse(this.tabs_form[index].orden.detalle[j].detalle_combo)).map((item) => (Number(item.cantidad)+' x '+item.nombre_producto))).join(', ');
                        }
                    }
                    var sumaTotal;
                    ...
                    if(!isNaN(sumaTotal)){
                        this.tabs_form[index].orden.subtotal = parseFloat(sumaTotal/1.18).toFixed(2);
        ...
        saveCombo(){
            let vm = this;
            vm.adicionalLoading = true;
            let detalle={
                id_producto: vm.temp_item.id_producto,
                cantidad: 1,
                precio: Number(vm.temp_item.precio).toFixed(2),
                observaciones: "",
                id_orden: this.tabs_form[this.tab].id_orden,
                detalle_combo: null,
            }
            let comboDetalle = []
            for (let index = 0; index < vm.comboDetail.length; index++) {
                const element = vm.comboDetail[index];
                if(element.id_producto){
                    //comboDetalle.push(element.producto)
                    comboDetalle.push({
                        id_producto: element.producto.id_producto,
                        nombre_producto: element.producto.nombre_producto,
                        cantidad: element.cantidad,
                        precio: '0.00'
                    });
                }
                else{
                    //comboDetalle.push(element.select)
                    comboDetalle.push({
                        id_producto: element.select.producto.id_producto,
                        nombre_producto: element.select.producto.nombre_producto,
                        cantidad: element.select.cantidad,
                        precio: element.select.precio
                    });
                }
            }
            detalle.detalle_combo = JSON.stringify(comboDetalle);
            console.log(detalle)
            axios.post('/api/orderdetail',detalle)
            .then((response) => {
                if(response.data.detalle_combo){
                   response.data.detalle_combo = JSON.parse(response.data.detalle_combo);
                    response.data.detalle_combo_descripcion = response.data.detalle_combo.map((item) => (Number(item.cantidad)+' x '+item.nombre_producto)).join(', ');
                }
                ...
                this.calcularTotal();
                vm.adicionalLoading = false;
                vm.adicionalDialog = false;
                this.comboDetalle = [];
                this.$store.commit('LOADER',false);
            })
            .catch( resonse => {
        ...
        addItemOrder(item){
            ...
            if(item.tipo_producto_combo==2){
                vm.temp_item = item;
                axios.get('/api/combo/'+item.id_producto)
                .then((response) => {
                    vm.comboDetail = response.data;
                    console.log(vm.comboDetail)
                    vm.adicionalDialog=true;
                })
                .catch( resonse => { 
                    this.$store.commit('LOADER',false);
                    vm.adicionalDialog=true;
        ...
        calcularTotal() {
            ...
            if(!isNaN(sumaTotal)){
                ...
                this.tabs_form[this.tab].orden.subtotal = parseFloat(sumaTotal/1.18).toFixed(2);
        ...
        preLoadData(){
            ...
            .then((response) => {
                ...
                this.generate_sales.header = JSON.parse(JSON.stringify(Object.assign(this.tabs_form[this.tab].orden, data)));
                this.generate_sales.header.id_tipo_comprobante = this.tipo_comprobante[2].id_tipo_comprobante;
                this.generate_sales.header.id_medio_pago = this.metodospago[0].id_medio_pago;
        ...
        saveComprobante(){
            ...
            for (let index = 0; index < element.length; index++) {
                ...
                this.generate_sales.detail.push({
                    ...
                    detalle_combo: JSON.stringify(orden_detalle.detalle_combo),
        ...
        printCocina(){
            ...
            axios.get('/api/itemsPrintCocina/'+id_orden)
            .then((response) => {
        ...
        async calcularVuelto(tab){
            ...
            //console.log(this.generate_sales.header.total_vuelto);
        },
