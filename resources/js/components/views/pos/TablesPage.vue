<template>
  <div class="flex-grow-1">
    <v-toolbar
          ...
          color="transparent"
        >
          <h2 style="color: #37474F">Mesas</h2> 
          <v-spacer></v-spacer>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon v-bind="attrs" v-on="on" :to="'/pickupSystem/'+id_sucursal">
    ...
    <div v-for="(n,i) in zona_mesas"
      ...
      >
        ...
        <v-col
          ...
          sm="2"
        >
          <v-card class="mb-4" light style="padding: 15px" :to="'/posSystem/'+t.id_mesa" color="#d61c1c" v-if="t.ordenes_count>0">
          <v-badge
          ...
          <v-card-title class="subtitle-2" style="padding: 0px; color: #fff;" v-text="t.nro_mesa"></v-card-title>
            
            <v-img
              src="/assets/images/chair-outline.png" style="max-width:150px;"
              class="d-flex align-center">
                
            </v-img>
          </v-card>

          <v-card class="mb-4" light style="padding: 15px" :to="'/posSystem/'+t.id_mesa" v-if="t.ordenes_count==0">
          <v-badge
              :content="t.ordenes_count"
              :value="t.ordenes_count"
              color="red"
          ...
          <v-card-title class="subtitle-2" style="padding: 0px;" v-text="t.nro_mesa"></v-card-title>
...
export default {
  ...
  methods: {
    ...
    getMesas(){
        axios.get('/api/getTableByBranch/'+this.$route.params.id)
        .then((response) => {
    ...
    async getCash(){
        try{
            const detail = await API.cash.detail(this.$route.params.id,1);
