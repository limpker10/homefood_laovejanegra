  ...
  <div class="flex-grow-1">
    ...
    <div>
      <v-card class="mb-4" light style="padding: 15px">
                      ...
                      <v-col cols="12" sm="8" md="8">
                        <v-container>
                            ...
                            <v-col cols="12" sm="6"  md="6" >
                                ...
                                ></v-text-field>
                                <v-text-field 
                                  :disabled="!(editedItem.tipo_producto_combo!=1)"
                            ...
                            <v-col cols="12" sm="12"  md="12" >
                                <v-select
                                    :disabled="!(editedItem.tipo_producto_combo==2 || editedItem.tipo_producto_combo==1)" 
        ...
        <v-dialog persistent v-model="dialogDelete" max-width="300px">
          <v-card>
            ...
            <v-card-text>
              <v-container>
                  <v-row dense class="pa-2 align-center">
                      ...
                      <v-icon>mdi-plus</v-icon> Producto
                      </v-btn>  

                      <v-btn small color="primary" class="mr-2" @click="dialogAddAdicionales=true;">
                      <v-icon>mdi-plus</v-icon> Opciones
                      </v-btn>           
                  </v-col>
                  </v-row>
                  <v-simple-table>
                      ...
                      <thead>
                          ...
                          <th class="text-left">Producto / Opciones</th>
                      ...
                      <tbody>
                          ...
                          :key="index"
                          >
                              <td>{{ item.id_producto ? 'PRODUCTO' : 'OPCIONES' }}</td>
                              <td v-if="item.id_producto">{{ item.producto.nombre_producto }}</td>
                              <td v-else>{{ item.adicional.nombre }}</td>
                              <td>{{ item.cantidad }}</td>
                              <td>
        ...
        <!--Add dialogAddAdicionales-->
        <v-dialog persistent 
          v-model="dialogAddAdicionales"
          max-width="550px"
        >
          <v-card>
            <v-form
              ref="formadicionales"
              v-model="valid_adicional"
              lazy-validation
            >
              <v-card-title>
                <span class="headline">{{ formTitleAdicionales }}</span>
              </v-card-title>
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12"  md="12" >
                      ...
                      placeholder="Elegir Bebidas"
                      :rules="requiredRules"
                      v-model="adicionales.nombre"
                      ></v-text-field>
                    </v-col>
                  </v-row>
                  <v-divider></v-divider>
                  <v-row>
                    ...
                    <v-col cols="2" sm="12"  md="2" >
                      <v-text-field 
                      v-model="model.precio_adicional" 
                      value="0.00"
                    ...
                    <v-col cols="1" sm="12"  md="1" style="margin: auto;">
                      <v-btn small icon @click="addProductoAdicional(model)">
                  ...
                  <v-list>
                    ...
                    <v-list-item
                      v-for="item in adicionales.productos_adicional"
                      :key="item.nombre_producto"
                    >
                      <v-list-item-content>
                        <v-list-item-title v-text="item.nombre"></v-list-item-title>

                        <v-list-item-subtitle v-text="'S/.'+item.precio"></v-list-item-subtitle>
                      </v-list-item-content>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="error" dark @click="dialogAddAdicionales=false" >Cancelar</v-btn>
                <v-btn color="primary" @click="saveAdicional()"> Guardar </v-btn>
              </v-card-actions>
            </v-form>
          </v-card>
        </v-dialog>

        <v-dialog persistent v-model="dialogDeleteCombinacion" max-width="300px">
          <v-card>
        ...
        <!--Editar Imagen-->
        <v-dialog persistent v-model="edit_image_dialog" max-width="500px">
          <v-card>
...
export default {
  ...
  computed: {
    ...
    formTitleAdicionales () {
        return this.editedIndexInsumo === -1 ? 'Agregar Adicionales' : 'Editar Adicionales'
    },
    fields() {
  ...
  watch: {
    ...
    model(val){
      let vm = this;
      vm.model.precio_adicional = '0.00';
      vm.model.cantidad = '1';
    }
  },
  ...
  methods: {
    ...
    editItem (item) {
        ...
        //console.log(this.sucursales_selected)
        this.editedItem.sucursales = this.editedItem.sucursal_producto.map(({ id_sucursal }) => id_sucursal)
        console.log(this.editedItem.sucursales)
        this.getSucursales();
        this.dialog = true;
    },
    ...
    addProductoAdicional(item){
      let vm = this;
      vm.adicionales.productos_adicional.push({
    ...
    async saveAdicional(){
      let vm = this;
      let id_combo = this.id_detail;
      vm.adicionales.id_combo = id_combo; 
      try {
        const data = await API.aditionals.create(vm.adicionales);
        console.log(data)
        this.dialogAddAdicionales = false;
        this.getDetail(this.editedInsumo.id_combo)
      } catch (error) {
        console.error(error)
      }
