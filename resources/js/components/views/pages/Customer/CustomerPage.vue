<template>
  <div class="flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <h2 style="color: #ad91fd">Clientes</h2>
                    ...
                    <v-row>
                      <v-col cols="12" sm="6" md="6">
                        ...
                        <v-text-field
                          ...
                          placeholder="Nombre / Razón Social Cliente"
                          :rules="requiredRules"
                        ></v-text-field>
                        <v-text-field
...
export default {
  ...
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: "ID", sortable: false, value: "id_cliente" },
  ...
  methods: {
    buscarDoc() {
      if (this.editedItem.id_tipo_doc == 2) {
        this.showName = false;
        axios
          .post("/api/searchDni", {
            dni: this.editedItem.nro_doc,
          })
          .then((response) => {
            if (response.data.success == true) {
              this.editedItem.nombre =
                response.data.result.Nombres +
                " " +
                response.data.result.Apellidos;
              this.showName = true;
        ...
        axios
          .post("api/searchRuc", {
            ruc: this.editedItem.nro_doc,
          })
          .then((response) => {
            if (response.data.success == true) {
              this.editedItem.nombre = response.data.nombre_o_razon_social;
              this.editedItem.direccion = response.data.direccion_completa;
    ...
    getlist(page, perpage) {
      ...
      axios
        .post("/api/getClientes?page=" + this.page, this.searchData)
        .then((response) => {
    ...
    deleteItemConfirm() {
      ...
      this.loading = true;
      axios
        .delete("/api/customer/" + this.editedItem.id_cliente, this.editedItem)
        .then((response) => {
          this.loading = false;
          this.getlist();
          this.closeDelete();
    ...
    save() {
      if (this.$refs.form.validate()) {
        ...
        if (this.editedIndex > -1) {
          ...
          axios
            .put("/api/customer/" + this.editedItem.id_cliente, this.editedItem)
            .then((response) => {
              this.loading = false;
              this.getlist();
              this.close();
          ...
          axios
            .post("/api/customer", this.editedItem)
            .then((response) => {
              this.loading = false;
