    ...
    <div>
      <v-card class="mb-4" light style="padding: 15px">
                ...
                <v-row>
                  <v-col cols="12" sm="12"  md="12" >
                    <v-select :items="sucursales"
                      ...
                      item-text="razon_social_sucursal" item-value="id_sucursal"
                      :rules="requiredRules"></v-select>
                    <v-select :items="zonas"
                      label="Zona" 
                      placeholder="Selecciona una zona" 
                      v-model="editedItem.id_zona"
                      item-text="nombre" item-value="id_zona"
                      :rules="requiredRules"></v-select>
                    <v-text-field
        ...
        <v-dialog v-model="dialogDelete" max-width="300px">
          <v-card>
            <v-card-title class="subtitle-1">¿Desea eliminar esta Mesa?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    getZonas(id_sucursal){
        axios.get('/api/branch/'+id_sucursal)
        .then((response) => {
    ...
    getlist(page,perpage) {
        ...
        axios.post('api/getMesas?page=' + this.page, this.searchData)
        .then(response => {
    ...
    changeStatus(id, status){
      ...
      axios.put('/api/changeStatus/'+id, {table: "mst_mesas",id:'id_mesa',status: status})
        .then(response => {
            this.loading = false;
            Toast.fire({
                icon: 'success',
    ...
    deleteItemConfirm () {
        ...
        this.loading = true;
        axios.delete('api/table/'+this.editedItem.id_mesa, this.editedItem)
            .then(response => {
                this.loading = false;
    ...
    save () {
        ...
        if (this.editedIndex > -1) {
            axios.put('api/table/'+this.editedItem.id_mesa, this.editedItem)
            .then(response => {
                this.loading = false;                
                this.getlist();
                this.close();
            ...
            axios.post('api/table', this.editedItem)
            .then(response => {
                this.loading = false;
                this.getlist();
                this.close();
