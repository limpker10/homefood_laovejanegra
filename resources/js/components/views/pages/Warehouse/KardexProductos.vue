<template>
  <div class="flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <h2 style="color: #37474F">Kárdex de Movimientos de Productos</h2> 
      ...
      <v-card class="mb-4" light style="padding: 15px">
        ...
        <v-data-table
            ...
            <template v-slot:[`item.cantidad`]="{ item }">
                <div>
                    <v-icon class="green--text darken-1" v-if="item.id_tipo_movimiento == 1 || item.id_tipo_movimiento == 2 || item.id_tipo_movimiento == 5">mdi-arrow-up</v-icon>
                    <v-icon class="red--text darken-2" v-if="item.id_tipo_movimiento == 3 || item.id_tipo_movimiento == 4">mdi-arrow-down</v-icon>
                    <strong class="green--text darken-1" v-if="item.id_tipo_movimiento == 1 || item.id_tipo_movimiento == 2 || item.id_tipo_movimiento == 5">{{item.cantidad}}</strong>
                    <strong class="red--text darken-2" v-if="item.id_tipo_movimiento == 3 || item.id_tipo_movimiento == 4">{{item.cantidad}}</strong>
                </div>
            </template>
            <template v-slot:[`item.stock_post`]="{ item }">
                <div v-if="item.id_tipo_movimiento == 1 || item.id_tipo_movimiento == 2 || item.id_tipo_movimiento == 5"> {{stock_post(item.stock_actual, item.cantidad, 0)}} </div>
                <div v-if="item.id_tipo_movimiento == 3 || item.id_tipo_movimiento == 4"> {{stock_post(item.stock_actual, item.cantidad, 1)}} </div>
            </template>
        </v-data-table>
      </v-card>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Stock Posterior', sortable: false, value: 'stock_post' },
  ...
  methods: {
    stock_post(stock, cantidad, n){
      let result = 0;
      if(n==0){
        result = Number(stock) + Number(cantidad);
      }
      else{
        result = Number(stock) - Number(cantidad);
      }
    ...
    getlist(page,perpage) {
        ...
        axios.post('api/getAlmacenProductos?page=' + this.page, this.searchData)
        .then(response => {
