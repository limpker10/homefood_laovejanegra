<template>
  <div class="flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <h2 style="color: #37474f">Stock de productos</h2>
        ...
        <v-card class="mb-4" light style="padding: 15px">
          ...
          >
            ...
            <template v-slot:[`item.unidad_medida`]="{ item }">
              <div>
                {{ item.producto.unidad_medida.simbolo }}
              </div>
            </template>
...
export default {
  ...
  methods: {
    editItem(item) {
      ...
      this.editedId = item.id_producto_sucursal;
    ...
    saveEdit() {
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          console.log(data);
          axios
            .post("/api/updateStockProducto", data)
            .then((res) => {
              this.$swal({
                toast: true,
                position: "top-end",
    ...
    async getlist(page, perpage) {
      ...
      try {
        ...
        const data = await API.items.stock(this.page, this.searchData);
        //const filter = data.filter((item) => item.producto.tipo_producto_combo == 3);
