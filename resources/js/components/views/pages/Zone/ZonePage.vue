<template>
  <div class="flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <h2 style="color: #37474F">Zonas</h2> 
      ...
      <v-card class="mb-4" light style="padding: 15px">
        ...
        >
            ...
            <template v-slot:[`item.sucursal`]="{ item }">
                <div>
                    {{item.sucursal.nombre_sucursal}} - {{item.sucursal.razon_social_sucursal}}
                </div>
            </template>
                ...
                <v-row>
                  <v-col cols="12" sm="12"  md="12" >
                    <v-text-field
                      ...
                      :rules="requiredRules"
                    ></v-text-field>
                    <v-select :items="sucursales"
                     label="Sucursal" 
                     placeholder="Selecciona una sucursal" 
                     v-model="editedItem.id_sucursal"
                      item-text="nombre_sucursal" item-value="id_sucursal"
        ...
        <v-dialog v-model="dialogDelete" max-width="300px">
          <v-card>
            <v-card-title class="subtitle-1">¿Desea eliminar esta Zona?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    dialogDelete: false,
    headers: [
      { text: 'ID', sortable: false, value: 'id_zona' },
      { text: 'Zona', sortable: false, value: 'nombre' },
  ...
  methods: {
    ...
    getlist(page,perpage) {
        ...
        axios.post('api/getZonas?page=' + this.page, this.searchData)
        .then(response => {
    ...
    changeStatus(id, status){
      ...
      axios.put('/api/changeStatus/'+id, {table: "zonas",id:'id_zona',status: status})
        .then(response => {
            this.loading = false;
    ...
    deleteItemConfirm () {
        ...
        this.loading = true;
        axios.delete('api/zone/'+this.editedItem.id_zona, this.editedItem)
            .then(response => {
                this.loading = false;
    ...
    save () {
      if(this.$refs.form.validate()){
        ...
        if (this.editedIndex > -1) {
            ...
            axios.put('api/zone/'+this.editedItem.id_zona, this.editedItem)
            .then(response => {
                this.loading = false;
            ...
            axios.post('api/zone', this.editedItem)
            .then(response => {
                this.loading = false;
                this.getlist();
                this.close();
