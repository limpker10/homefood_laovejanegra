  ...
  <div>
    ...
    <v-dialog v-model="dialog" max-width="500px">
        ...
        <v-card-actions>
            ...
            <v-btn color="primary" class="mr-2" @click="update(); edit_loading=true;" :loading="edit_loading">Guardar cambios</v-btn>
        </v-card-actions>
        </v-card>
    </v-dialog>
    <!--eliminar permiso-->
    <v-dialog v-model="deleteDialog" max-width="290">
        <v-card>
            <v-card-title class="headline">Eliminar Permiso</v-card-title>
            <v-card-text>¿Está seguro de que desea eliminar este permiso?</v-card-text>
            <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn @click="deleteDialog = false">Cancelar</v-btn>
            <v-btn color="error" @click="deletePermission(); delete_loading=true;" :loading="delete_loading">Eliminar</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async update () {
        try{
            const permission = await API.permissions.update( this.editedItem.id, this.editedItem );
            this.loading = false;
            this.validate();
            this.close();
            this.$swal({
    ...
    async deletePermission() {
        this.isLoading=true;
        try {
            const response = await API.permissions.remove(this.editedItem.id);
            this.$swal({
                toast: true,
                position: 'top-end',
                icon:'success',
