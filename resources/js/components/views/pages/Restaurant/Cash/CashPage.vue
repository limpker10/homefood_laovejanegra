<template>
    <div class="flex-grow-1">
        <div class="d-flex align-center py-3">
            <h2 style="color:  #AD91FD">Caja {{$route.params.id == 'hotel' ? 'Hotel':'Restaurante'}}</h2>
        </div>
        ...
        <v-card class="mb-4" light style="padding: 15px">
            <v-data-table
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <v-btn v-if="item.fecha_cierre == null" small icon @click="getCash(item.id_caja)">
...
export default {
    data: () => ({
        ...
        headers: [
            { text: '#', sortable: false, value: 'id_caja' },
